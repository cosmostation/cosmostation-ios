...
//
    ...
    static func onParseVestingAccount(_ baseChain: BaseChain) {
        guard let grpcfetcher = baseChain.getGrpcfetcher(),
              let authInfo = grpcfetcher.cosmosAuth else {
            return
        }
           ...
           let vestingAccount = try? Cosmos_Vesting_V1beta1_PeriodicVestingAccount.init(serializedData: authInfo.value) {
            ...
            grpcfetcher.cosmosBalances?.forEach({ coin in
                ...
                if (dpVesting.compare(NSDecimalNumber.zero).rawValue > 0) {
                    ...
                    grpcfetcher.cosmosVestings.append(vestingCoin)
                    var replace = -1
                    for i in 0..<(grpcfetcher.cosmosBalances?.count ?? 0) {
                        if (grpcfetcher.cosmosBalances![i].denom == denom) {
                            replace = i
                        }
                    }
                    if (replace >= 0) {
                        grpcfetcher.cosmosBalances![replace] = Cosmos_Base_V1beta1_Coin.with {  $0.denom = denom; $0.amount = dpBalance.stringValue }
                    }
                }
            })
