    ...
    func onBindView(_ myPools: Array<Osmosis_Gamm_Balancer_V1beta1_Pool>, _ lockUps: Array<Osmosis_Lockup_PeriodLock>) {
        ...
        lockUps.forEach { lockup in
            ...
            if let matchedPool = myPools.filter({ $0.id == lpCoin.osmosisAmmPoolId() }).first {
                ...
                let coin0Value = WUtils.usdValue(BaseData.instance.getBaseDenom(chainConfig, denom0), withdrawableAmount0, WUtils.getOsmosisCoinDecimal(denom0))
                let coin1Value = WUtils.usdValue(BaseData.instance.getBaseDenom(chainConfig, denom1), withdrawableAmount1, WUtils.getOsmosisCoinDecimal(denom1))
