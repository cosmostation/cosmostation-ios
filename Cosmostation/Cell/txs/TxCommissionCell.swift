...
class TxCommissionCell: TxCell {
    ...
    override func onBindMsg(_ chain: ChainType, _ response: Cosmos_Tx_V1beta1_GetTxResponse, _ position: Int) {
        ...
        if (commissionsCoins.count > 0) {
            ...
            WUtils.showCoinDp(commissionsCoins[0], incen0Denom, incen0Amount, chain)
        }
        if (commissionsCoins.count > 1) {
            incen1Layer.isHidden = false
            WUtils.showCoinDp(commissionsCoins[1], incen1Denom, incen1Amount, chain)
        }
        if (commissionsCoins.count > 2) {
            incen2Layer.isHidden = false
            WUtils.showCoinDp(commissionsCoins[2], incen2Denom, incen2Amount, chain)
        }
        if (commissionsCoins.count > 3) {
            incen3Layer.isHidden = false
            WUtils.showCoinDp(commissionsCoins[3], incen3Denom, incen3Amount, chain)
        }
