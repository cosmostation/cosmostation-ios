...
let KEY_DISPLAY_GRC20_TOKENS            = "KEY_DISPLAY_GRC20_TOKENS"
...
let KEY_DROP_POPUP                      = "KEY_DROP_POPUP"
let KEY_DYDX_POPUP                      = "KEY_DYDX_POPUP"
let KEY_BABYLON_POPUP                   = "KEY_BABYLON_POPUP"
...
let DB_ACCOUNT_ID                   = SQLite.Expression<Int64>("id")
let DB_ACCOUNT_UUID                 = SQLite.Expression<String>("uuid")
let DB_ACCOUNT_NICKNAME             = SQLite.Expression<String>("nickName")
let DB_ACCOUNT_FAVO                 = SQLite.Expression<Bool>("isFavo")
let DB_ACCOUNT_ADDRESS              = SQLite.Expression<String>("address")
let DB_ACCOUNT_BASECHAIN            = SQLite.Expression<String>("baseChain")
let DB_ACCOUNT_HAS_PRIVATE          = SQLite.Expression<Bool>("hasPrivateKey")
let DB_ACCOUNT_RESOURCE             = SQLite.Expression<String>("resource")
let DB_ACCOUNT_FROM_MNEMONIC        = SQLite.Expression<Bool>("fromMnemonic")
let DB_ACCOUNT_PATH                 = SQLite.Expression<String>("path")
let DB_ACCOUNT_IS_VALIDATOR         = SQLite.Expression<Bool>("isValidator")
let DB_ACCOUNT_SEQUENCE_NUMBER      = SQLite.Expression<Int64>("sequenceNumber")
let DB_ACCOUNT_ACCOUNT_NUMBER       = SQLite.Expression<Int64>("accountNumber")
let DB_ACCOUNT_FETCH_TIME           = SQLite.Expression<Int64>("fetchTime")
let DB_ACCOUNT_M_SIZE               = SQLite.Expression<Int64>("msize")
let DB_ACCOUNT_IMPORT_TIME          = SQLite.Expression<Int64>("importTime")
let DB_ACCOUNT_LAST_TOTAL           = SQLite.Expression<String>("lastTotal")
let DB_ACCOUNT_SORT_ORDER           = SQLite.Expression<Int64>("sortOrder")
let DB_ACCOUNT_PUSHALARM            = SQLite.Expression<Bool>("pushAlarm")
let DB_ACCOUNT_NEW_BIP              = SQLite.Expression<Bool>("newBip")            //using alternative ket gen path or type(OKex)
let DB_ACCOUNT_CUSTOM_PATH          = SQLite.Expression<Int64>("customPath")
let DB_ACCOUNT_MNEMONIC_ID          = SQLite.Expression<Int64>("mnemonic_id")


//DB for Mnemonic
let DB_MNEMONIC = Table("mnemonic")
let DB_MNEMONIC_ID                  = SQLite.Expression<Int64>("id")
let DB_MNEMONIC_UUID                = SQLite.Expression<String>("uuid")
let DB_MNEMONIC_NICKNAME            = SQLite.Expression<String>("nickName")
let DB_MNEMONIC_CNT                 = SQLite.Expression<Int64>("wordsCnt")
let DB_MNEMONIC_FAVO                = SQLite.Expression<Bool>("isFavo")
let DB_MNEMONIC_IMPORT_TIME         = SQLite.Expression<Int64>("importTime")


//V2 DB BaseAccount
let TABLE_BASEACCOUNT       = Table("BaseAccount")
let BASEACCOUNT_ID          = SQLite.Expression<Int64>("id")
let BASEACCOUNT_UUID        = SQLite.Expression<String>("uuid")
let BASEACCOUNT_NAME        = SQLite.Expression<String>("name")
let BASEACCOUNT_TYPE        = SQLite.Expression<Int64>("type")
let BASEACCOUNT_LAST_PATH   = SQLite.Expression<String>("lastpath")
let BASEACCOUNT_ORDER       = SQLite.Expression<Int64>("order")

//V2 DB Ref_Address(derived address from mnonics or privatekey)
let TABLE_REFADDRESS        = Table("BaseAddress")
let REFADDRESS_ID           = SQLite.Expression<Int64>("id")
let REFADDRESS_ACCOUNT_ID   = SQLite.Expression<Int64>("accountId")
let REFADDRESS_CHAIN_TAG    = SQLite.Expression<String>("chainTag")
let REFADDRESS_DP_ADDRESS   = SQLite.Expression<String>("dpAddress")
let REFADDRESS_EVM_ADDRESS  = SQLite.Expression<String>("evmAddress")
let REFADDRESS_MAIN_AMOUNT  = SQLite.Expression<String>("lastMainAmount")
let REFADDRESS_MAIN_VALUE   = SQLite.Expression<String>("lastMainAValue")
let REFADDRESS_TOKEN_VALUE  = SQLite.Expression<String>("lastTokenAValue")
let REFADDRESS_COIN_CNT     = SQLite.Expression<Int64>("lastCoinCnt")

//V2 DB Ref_Address
let TABLE_ADDRESSBOOK       = Table("AddressBook")
let ADDRESSBOOK_ID          = SQLite.Expression<Int64>("id")
let ADDRESSBOOK_NAME        = SQLite.Expression<String>("bookName")
let ADDRESSBOOK_CHAIN_NAME  = SQLite.Expression<String>("chainName")
let ADDRESSBOOK_ADDRESS     = SQLite.Expression<String>("address")
let ADDRESSBOOK_MEMO        = SQLite.Expression<String>("memo")
let ADDRESSBOOK_TIME        = SQLite.Expression<Int64>("lasttime")
