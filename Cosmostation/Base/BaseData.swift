...
extension BaseData {
    ...
    func setDisplayGrc20s(_ id: Int64, _ chainTag: String, _ contractAddress: [String])  {
        if let encoded = try? JSONEncoder().encode(contractAddress) {
            UserDefaults.standard.setValue(encoded, forKey: String(id) + " " + chainTag + " " + KEY_DISPLAY_GRC20_TOKENS)
        }
    }
    
    func getDisplayGrc20s(_ id: Int64, _ chainTag: String) -> [String]? {
        if let savedData = UserDefaults.standard.object(forKey: String(id) + " " + chainTag + " " + KEY_DISPLAY_GRC20_TOKENS) as? Data {
            if let result = try? JSONDecoder().decode([String].self, from: savedData) {
                return result
            }
        }
