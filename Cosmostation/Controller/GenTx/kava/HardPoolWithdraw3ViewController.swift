//
//  HardPoolWithdraw3ViewController.swift
//  Cosmostation
//
...
class HardPoolWithdraw3ViewController: BaseViewController, PasswordViewDelegate {
    ...
    func onGenWithdrawHardPoolTx() {
        DispatchQueue.global().async {
            let msg = MsgGenerator.genWithdrawHardMsg(self.chainType!,
            ...
            DispatchQueue.main.async(execute: {
                ...
                do {
                    ...
                    request.responseJSON { response in
                        ...
                        switch response.result {
                        case .success(let res):
                            print("HardPoolWithdraw ", res)
                            if let result = res as? [String : Any]  {
                                txResult = result
                            }
                        case .failure(let error):
                            print("HardPoolWithdraw error ", error)
