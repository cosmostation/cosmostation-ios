...
class CosmosRedelegate: BaseVC {
    ...
    func onSimul() {
        ...
        Task {
            do {
                if let simulReq = try await Signer.genSimul(selectedChain, onBindRedelegateMsg(), txMemo, txFee, txTip),
...
extension CosmosRedelegate: BaseSheetDelegate, MemoDelegate, AmountSheetDelegate, PinDelegate {
    ...
    func onPinResponse(_ request: LockType, _ result: UnLockResult) {
        if (result == .success) {
            ...
            Task {
                do {
                    if let broadReq = try await Signer.genTx(selectedChain, onBindRedelegateMsg(), txMemo, txFee, txTip),
