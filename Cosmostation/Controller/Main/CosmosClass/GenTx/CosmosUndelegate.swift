    ...
    func onSimul() {
        ...
        Task {
            do {
                ...
                let simulReq = Signer.genUndelegateSimul(account!, UInt64(height), toUndelegate, txFee, txMemo, selectedChain)
        ...
        if (result == .success) {
            ...
            Task {
                do {
                    let account = try await grpcFetcher.fetchAuth()
                    let height = try await grpcFetcher.fetchLastBlock()!.block.header.height
                    let broadReq = Signer.genUndelegateTx(account!, UInt64(height), toUndelegate, txFee, txMemo, selectedChain)
