...
class CosmosCompounding: BaseVC {
    ...
    func onSimul() {
        ...
        Task {
            do {
                if let simulReq = try await Signer.genSimul(selectedChain, onBindCompoundingMsg(), txMemo, txFee, txTip),
...
extension CosmosCompounding: MemoDelegate, BaseSheetDelegate, PinDelegate {
    ...
    func onPinResponse(_ request: LockType, _ result: UnLockResult) {
        if (result == .success) {
            ...
            Task {
                do {
                    if let broadReq = try await Signer.genTx(selectedChain, onBindCompoundingMsg(), txMemo, txFee, txTip),
