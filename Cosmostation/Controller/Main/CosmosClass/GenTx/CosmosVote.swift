    ...
    func onSimul() {
        ...
        Task {
            do {
                ...
                let simulReq = Signer.genVotesSimul(account!, UInt64(height), toVote, txFee, txMemo, selectedChain)
        ...
        if (result == .success) {
            ...
            Task {
                do {
                    let account = try await grpcFetcher.fetchAuth()
                    let height = try await grpcFetcher.fetchLastBlock()!.block.header.height
                    let broadReq = Signer.genVotesTx(account!, UInt64(height), toVote, txFee, txMemo, selectedChain)
