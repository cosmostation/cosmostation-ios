...
class NeutronOverruleDao: BaseVC {
    ...
    func fetchProposals() {
        ...
        Task {
            if let response = try await neutronFetcher.fetchNeutronProposals(2),
...
extension NeutronOverruleDao: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let module = selectedChain.neutronFetcher!.daosList?[0]["proposal_modules"][2]
        var proposal: JSON!
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        let contAddress = selectedChain.neutronFetcher!.daosList?[0]["proposal_modules"][2]["address"].string ?? ""
        var proposal: JSON!
        if (indexPath.section == 0) {
            if (isShowAll) {
                proposal = votingPeriods[indexPath.row]
